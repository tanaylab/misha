...
* Added `gdb.build_genome(name)` for build-from-source genome creation. End-to-end validated on UCSC `sacCer3` and `dm6` (full pipeline including `tss`/`exons`/`utr3`/`utr5`/`rmsk`/`cpgIsland`/`cytoband`) and NCBI Datasets `GCF_000913595.1`. Resolves a name through a registry chain (explicit arg, `getOption("misha.genome_registry")`, project-local `misha.yaml`, built-in `inst/genomes.yaml`) and dispatches to one of: `ucsc` (golden path FASTA + `ncbiRefSeq` genes + `rmsk`/`cpgIslandExt`/`cytoBandIdeo`), `ncbi` (Datasets API FASTA + GFF3, requires UCSC's `gff3ToGenePred` binary), `s3` (alias for `gdb.create_genome`), `local` (`gdb.init` an existing groot), or `manual` (registry-supplied URLs). Names matching `^GC[FA]_\d+\.\d+$` are resolved to the NCBI backend without a registry entry. Companion functions: `gdb.list_genomes()`, `gdb.genome_info()`, `gdb.install_gff3_converter()`. Writes a `genome_info.yaml` at the groot root recording the source recipe and download time.
* Behavior delta vs. `gdb.create_genome("hg38")`: the S3 tarball ships gene tracks built from UCSC `knownGene`+`kgXref`; `gdb.build_genome("hg38")` builds them from RefSeq (`ncbiRefSeq`+`ncbiRefSeqLink`). Different gene sets — pick whichever your downstream expects.
* Known limitation: UCSC's prebuilt `gff3ToGenePred` binary requires glibc ≥ 2.34. On older systems (e.g. CentOS 8), set `MISHA_GFF3_TO_GENEPRED` to a binary from `conda install -c bioconda ucsc-gff3togenepred`.
