...
* New optional `target_chroms` argument on `gdb.build_genome()` and `gdb.install_intervals()`. Pass a character vector of chrom names the build should align to (typically `halStats --sequenceStats` output for a HAL you'll liftover against) and misha auto-picks the chromAlias column whose values cover those names best, removing the need to hand-pick `chrom_naming = "genbank"` vs `"ucsc"`. Default on `gdb.install_intervals()` is the groot's own chroms (existing behavior); default on `gdb.build_genome()` is `NULL` (use `chrom_naming` or backend default). An optional companion `target_lengths` (aligned with `target_chroms`) lets a column that falls just short of `min_coverage` get rescued via unique-length-pair fill against the supplied lengths -- useful when the target naming has empty cells in the chromAlias (e.g. an MT contig missing a `genbank` accession).
