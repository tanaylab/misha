...
* `gdb.build_genome()` / `gdb.install_intervals()` (ucsc-hub source) now also fetch and merge `<acc>_assembly_report.txt` when the hub mirrors it. NCBI's per-sequence report often carries rows the chromAlias drops (typically unplaced scaffolds) plus the `UCSC-style-name` column, which lifts chromAlias coverage from ~97% to ~99.9% for roughly 30 of the 241 Zoonomia hub assemblies (e.g. `GCF_000001895.5` rat). No new arguments; the merge is automatic when the report is present and a no-op when absent.
