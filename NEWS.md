...
* Added `getOption("gmultitasking.strategy")` for `gextract` (default `"auto"`). When the workload is large and many-track, `auto` routes to a track-parallel mode (each `parallel::mclapply` worker handles a track subset across all tiles) instead of the legacy tile-parallel mode (each fork-kid handles a tile range across all tracks). On cold-NFS many-track gextract, track-parallel measured ~5× faster (n106 head-to-head: 69.6 s → 13.1 s for 30 tracks × 2.19M bins) because each worker only mmap-faults its own files, eliminating the working-set thrashing that 24 kids × 500 tracks creates on the kernel page cache. Override per-call via `options(gmultitasking.strategy = "tracks" | "tiles" | "auto")`. Default heuristic stays on `"tiles"` for single-track / small-workload / file-output / 2D queries so nothing else regresses.
