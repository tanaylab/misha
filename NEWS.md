...
* Switched dense-track `MmapFile` from `MADV_SEQUENTIAL` to `MADV_RANDOM`. Cold-NFS gextract over many dense tracks with a tiled iterator is **30× faster** (measured: 159s → 5s for 100 motif tracks × 998 tile bins on lab cluster; warm cache unchanged). `MADV_SEQUENTIAL` was triggering a synchronous read-ahead window per page fault that wasted bandwidth — for tile-clustered access we touch a few KB out of each ~50 MB chrom file. `MADV_RANDOM` disables read-ahead entirely; on-demand 4 KB page reads are vastly cheaper than over-fetching. Sequential scans (e.g. `gextract` of a single dense track over a full chromosome with `iterator=50`) remain just as fast since the data is `memcpy`'d from already-resident pages. All 9 perf-regression baselines pass.
