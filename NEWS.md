...
* Fixed: `gdb.install_intervals()` could produce a `xlat.genePred` (or analogous translated rmsk/cgi/cytoband data) whose CHROM column was empty for some rows, tripping the C++ importer's "line N: invalid file format" check. This happened when an asset's source chrom name lived in the chromAlias but the matching alias row's canonical was unset by the 3-pass resolution (e.g. an MT transcript whose accession is in chromAlias but the groot has no MT contig). Empty-chrom rows are now dropped from each translated asset, with a verbose count. As a defensive fix the genePred reader also pins `exonStarts`/`exonEnds` to character so a single-exon-only input doesn't get type-inferred to numeric and lose the trailing comma the C++ importer requires.
* `gdb.build_genome()` / `gdb.install_intervals()` (ucsc-hub source) now also fetch and merge `<acc>_assembly_report.txt` when the hub mirrors it. NCBI's per-sequence report often carries rows the chromAlias drops (typically unplaced scaffolds) plus a `UCSC-style-name` column whose values frequently differ from chromAlias's `ucsc` column (chromAlias uses RefSeq-derived names like `chr1_NW_xxx_random`; the report uses GenBank-derived `chr1_AABRxxx_random`). The merge keeps every report column as a separate alias column under a normalized name (e.g. `ucsc_style_name`, `sequence_role`, `sequence_length`), so `.detect_alias_column` scores each naming scheme independently and the one that matches the groot wins. Lifts coverage from ~97% to ~99.9% for roughly 30 of the 241 Zoonomia hub assemblies (e.g. `GCF_000001895.5` rat, `GCF_000002285.5` dog). No new arguments; the merge is automatic when the report is present, a no-op when absent.
...
* New optional `target_chroms` argument on `gdb.build_genome()` and `gdb.install_intervals()`. Pass a character vector of chrom names the build should align to (typically `halStats --sequenceStats` output for a HAL you'll liftover against) and misha auto-picks the chromAlias column whose values cover those names best, removing the need to hand-pick `chrom_naming = "genbank"` vs `"ucsc"`. Default on `gdb.install_intervals()` is the groot's own chroms (existing behavior); default on `gdb.build_genome()` is `NULL` (use `chrom_naming` or backend default).
* New optional `target_lengths` argument on `gdb.build_genome()` (aligned with `target_chroms`, typically the second field of `halStats --sequenceStats`). When supplied with `match_by_length = TRUE`, misha **force-aligns** the hub FASTA to `target_chroms`: every target is placed on a chromAlias row by name match (across any column) or unique-on-both-sides length pairing, and the FASTA is renamed outright. If any target can't be placed, the build errors in the pre-flight before the FASTA download. On success the resulting groot's chrom names are exactly `target_chroms`, so e.g. HAL chrom names can be the canonical names in the groot even when they don't appear in the standard UCSC/GenBank/RefSeq columns.
* New `match_by_length` argument on `gdb.install_intervals()` (and `gdb.build_genome()`), default `TRUE`. Complements column-based canonical detection with a per-row length match (downloads `<acc>.chrom.sizes.txt` from the UCSC hub) for alias rows the chosen column couldn't cover -- typical for assemblies whose `genbank` column is empty on `chrM`. Asset translation also switches to a cross-column per-row lookup, so a GFF in any naming scheme (or a mix of schemes) imports cleanly. Length matches require the length to be unique on both sides; ambiguous lengths are skipped, never guessed. Set `FALSE` for the stricter single-column-only behavior.
* `chromAlias` coverage is now bp-weighted on the groot side. Previously it was the fraction of distinct chrom names; a long-tail of small unmapped contigs (e.g. a 16 kb mitochondrion missing from UCSC's `genbank` column) now costs ~0.0005% instead of ~1/N. Error and progress messages report bp coverage. Asset-side detection (chroms read from a GTF/GFF) stays count-weighted since lengths aren't available there.
* `gdb.build_genome()` gains a `min_coverage` argument (forwarded to `gdb.install_intervals()`), so single-call builds can relax strict 100% chromAlias coverage when a target column has small gaps (typical: UCSC's `genbank` column has no value for the mitochondrial contig, leaving 1 stray chrom).
* `gdb.build_genome()` (`ucsc-hub` source): chromAlias coverage is now checked before the FASTA download. Builds where no chromAlias column meets `min_coverage` against the resolved canonical column now fail in seconds (after ~190 KB of `chromAlias.txt` + a few KB of `chrom.sizes.txt`) instead of after the multi-GB FASTA transfer.
* Fixed: `chrom_naming` against a chromAlias column with empty cells (e.g. `genbank` blank for `chrM` in many UCSC hubs) used to rewrite the FASTA header to `>` (empty), which misha's loader silently surfaced as a phantom chrom called `contig`. Empty target values are now treated as "no rename", so the affected contig keeps its original FASTA id.
* `gdb.install_intervals(source = list(source = "ncbi", ...), sets = c("genes", ...))` now pre-flights the NCBI Datasets `dataset_report` endpoint (a few KB) before downloading the ~800 MB FASTA+annotation zip. If the assembly has no annotation (typical for community submissions: Sanger TOL, Zoonomia GCAs, ~80% of Phylo447), `genes` is dropped from the request with an actionable warning; if a RefSeq companion exists for the same taxon (e.g. `GCA_000003025.7` -> `GCF_000003025.6`) the warning suggests it.
...
* Added `gsynth.train`, `gsynth.sample` and `gsynth.save` functions that train a Markov model from a genome sequence and sample a synthetic genome from the model
* Added `gseq.kmer.dist` function that counts the number of occurrences of k-mers in genomic intervals.
* Fixed a bug in `gtrack.liftover` that created overlapping intervals when lifting sparse tracks.
