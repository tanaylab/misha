...
* **Bug fix:** `gquantiles()` returned wrong values - zeroes at the low percentiles - or aborted the session, when it ran across processes on a scope holding more values than `gmax.data.size` but fewer than `gquantile.edge.data.size` (100000 by default).
* **Crash fix:** catching a warning from `gquantiles()`, `gintervals.quantiles()`, `gbins.quantiles()`, or from loading an intervals set that contains zero-length intervals - with `tryCatch(warning = )`, `options(warn = 2)`, or `testthat::expect_warning()` - left the session broken: every later multitasking call died with "Child process ended unexpectedly", and the overlapping-iterator warning fell silent. The warnings themselves are unchanged.
