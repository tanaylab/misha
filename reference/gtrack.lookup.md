...
that are used in turn to retrieve values from a lookup table and create
a track.
...
is then used to address the lookup table and create with its values a
new track. More than one 'expr'-'breaks' pair can be used. In that case
...
If 'include.lowest' is 'TRUE' the the lowest value is included in the
...
the value in the track would be 'NaN' too.
