...
#' chromosome alias chain (\code{chr1} <-> \code{1}, \code{M} <-> \code{MT},
#' and \code{<groot>/chrom_aliases.tsv} if present). If none of the names in
#' the file can be matched, 'gtrack.import' fails instead of creating an empty
#' track. If only some of them can, the rest of the file is imported and the
#' unmatched names are reported: a message for contigs the database does not
#' have (unplaced scaffolds, patches), a warning when the unmatched name looks
#' like a primary chromosome, since that suggests a naming mismatch.
#'
    ...
    tryCatch(
        {
            ...
            .gtrack.create_atomic(trackstr, function() {
                if (used_bed) {
                    ...
                    # file may be a temporary unzipped / bigWig-converted copy that is
                    # deleted before this call returns; file.original is what the user
                    # asked for, and is what C++ names in its diagnostics.
                    .gcall("gtrackimportwig", trackstr, file, file.original, binsize, defval, .misha_env())
                }
            })
