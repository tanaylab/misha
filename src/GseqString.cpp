...
static ScoreResult score_pwm_over_range(
    ...
    int best_start0 = -1;
    int best_strand = 1;
    double total = R_NegInf;  // Start with -Inf for log-sum-exp
    int count = 0;
    ...
    for (int s0 = start_min0; s0 <= start_max0; ++s0) {
        ...
        if (mode == "lse") {
            // Use log-sum-exp to accumulate scores properly
            if (bidirect) {
                // For bidirectional, add both strands using log-sum-exp
                total = log_sum_exp_add(total, fwd_score);
                total = log_sum_exp_add(total, rev_score);
            } else {
                // For single strand, add the score from that strand
            ...
            // Track best position and strand
    ...
    if (mode == "lse") {
        ...
        if (best_start0 >= 0) {
            result.has_pos = true;
            result.pos_1based = best_start0 + 1;  // convert to 1-based
    ...
    try {
        ...
        for (int i = 0; i < n_seqs; ++i) {
            ...
            ScoreResult result = score_kmer_over_range(
                seq, start_min0, start_max0, kmer, strand_mode
            );
