...
- `gdb.install_intervals(groot, source, sets, prefix, ...)` - install / refresh `intervs.global.*` annotation sets into an existing groot from an upstream source (UCSC, UCSC track hub, NCBI, local files, S3 backend). `sets =` is a whitelist (default `c("genes", "rmsk", "cgi", "cytoband")`); `prefix` lets you namespace alternative versions.
...
- `gsynth.score(model, track, description = NULL, intervals = NULL, mask = NULL, resolution = NULL, ...)` - materialize the model's per-bin log-probability as a dense misha track (`track =` is the new track name; `resolution =` defaults to the model's training iterator). Always pass `description =` / `intervals =` by name - they're optional and easy to confuse if passed positionally.
