...
**A21. Verifying a `pwm.max` hit by re-scoring the span with `gseq.pwm(mode = "max")`.** They are not the same function under `bidirect = TRUE`: the vtrack combines the strands per position with LSE and then maxes over positions, while `gseq.pwm(mode = "max")` returns the plain cross-strand max. The check fails by up to `log 2`, and by exactly `log 2` on a palindrome - which reads like an off-by-one in your span arithmetic and sends you hunting in the wrong place. On a span exactly `nrow(pssm)` long there is a single window, so `gseq.pwm(mode = "lse")` is the equivalent; otherwise re-score with the same vtrack under `extend = FALSE`.
