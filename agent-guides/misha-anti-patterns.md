...
**A14b. `order(intervalID)` back into a frame you built with `gintervals()`.** `gintervals()` sorts its input (it ends in a `gintervsort` call), so `intervalID` indexes the *sorted* frame, not the one you handed it. A9 and A14 therefore interact: build an intervals frame in a meaningful non-genomic order (sampled rows, paired controls, an order you intend to index positionally), pass it through `gintervals()`, then `cbind` `res$value[order(res$intervalID)]` back onto your original frame, and you get a silent row shuffle. A plain `data.frame` round-trips (its `intervalID`s come back permuted and `order()` undoes it); the `gintervals()` one does not. Either sort your own frame first so the two orders agree, or carry your own id column and join on it - never index positionally from `order(intervalID)` into a frame you did not sort yourself.
...
**A20. `bidirect = TRUE` when motif orientation matters.** Bidirectional PWM scoring combines the two strands per position with a log-sum-exp (not a max), and discards which strand won. For oriented-motif analyses (CTCF orientation, stranded peak calling), pair two vtracks with `strand = +1` and `strand = -1`, `bidirect = FALSE`, so the orientation survives.

**A21. Verifying a `pwm.max` hit by re-scoring a span that is not the vtrack's window set.** Since misha 5.12.0 `gseq.pwm(mode = "max")` and `pwm.max` take the same strand union - LSE per position, then max over positions - so they do agree, but only window-for-window. The vtrack's `extend = TRUE` (the default) pads the interval's **END** by `nrow(pssm) - 1`, so `gseq.extract(iv)` on its own is short by that much and silently drops the last `nrow(pssm) - 1` anchors. Extract `gintervals(chrom, start, end + nrow(pssm) - 1)` and score that with `extend = FALSE`, or re-score with the same vtrack under `extend = FALSE`. Same rule for `gseq.pwm(mode = "count")` against `pwm.count`. Before 5.12.0 the two also disagreed on the strand union itself, by up to `log 2` (exactly `log 2` on a palindrome), and `gseq.pwm(mode = "count")` counted a self-reverse-complementary window twice.
