...
\begin{tabular}{|l|p{2in}|p{2in}|}
  ...
  Object names retrieval & \texttt{gtrack.attr.export} & \texttt{gtrack.var.ls} \\
...
Multiplication is a good example of "parallel" operation in R (works on each element in vector separately). On contrary some functions that accept a vector might return a scalar rather than a vector. Such is, for example, \texttt{min} function.
