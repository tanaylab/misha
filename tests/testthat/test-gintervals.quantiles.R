...
test_that("gintervals.quantiles works with intervals.set.out", {
    gintervals.rm("temp.testintervs_quantiles", force = TRUE)
    withr::defer(gintervals.rm("temp.testintervs_quantiles", force = TRUE))
    intervs1 <- gscreen("test.fixedbin > 0.2 & test.fixedbin < 0.3", gintervals(c(1, 2, 3), 0, -1))
    intervs2 <- gscreen("test.fixedbin > 0.25 & test.fixedbin < 0.35", gintervals(c(1, 2), 0, -1))
    set.seed(60427)
    intervs2 <- intervs2[sample(nrow(intervs2)), ]
    gintervals.quantiles("test.fixedbin", c(0.5, 0.2, 0.9), intervals = intervs2, iterator = intervs1, intervals.set.out = "temp.testintervs_quantiles")
    expect_equal(
        gintervals.load("temp.testintervs_quantiles"),
...
test_that("gintervals.quantiles works with intervals.set.out (2d)", {
    ...
    set.seed(60427)
    intervs <- intervs[sample(nrow(intervs)), ]
    gintervals.quantiles("test.rects", percentile = c(0.5, 0.3, 0.2, 0.9), intervs, iterator = c(1, 1), intervals.set.out = "temp.testintervs_quantiles")
    expect_equal(
        gintervals.load("temp.testintervs_quantiles"),
