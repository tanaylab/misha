...
test_that("gscreen with character vector returns long data.frame", {
    ...
    expect_true(all(res$track %in% c("dense_track > 0.15",
                                     "dense_track < 0.05")))
})
...
test_that("gscreen multi-track fast path matches per-track legacy calls", {
    ...
    ref <- do.call(rbind, lapply(exprs, function(e) {
        iv <- gscreen(e)
        if (nrow(iv) == 0) {
            data.frame(chrom = character(0), start = numeric(0),
    ...
    fast_s <- ord(fast); rownames(fast_s) <- NULL
    ref_s  <- ord(ref);  rownames(ref_s)  <- NULL
...
test_that("gscreen supports all five comparison operators", {
    ...
    for (op in c("<", "<=", "==", ">=", ">")) {
        ...
        exprs <- c(paste("dense_track", op, "0.1"),
