test_that("gterator.cartesian_grid works (1)", {
    ...
    expect_regression(giterator.intervals("1", ALLGENOME, iterator = itr), "giterator.cartesian_grid.1")
})
...
test_that("gterator.cartesian_grid works with band", {
    ...
    itr <- giterator.cartesian_grid(intervs1, c(-100000, -50000, -10000, 20000, 700000), intervs2, c(-200000, -30000, -10000, 60000, 100000, 200000))
    expect_regression(giterator.intervals("1", ALLGENOME, iterator = itr, band = c(-20000, 30000)), "giterator.cartesian_grid.band")
    expect_error(giterator.intervals("1", ALLGENOME, iterator = itr, band = c(1, 1)))
})
...
test_that("gterator.cartesian_grid works with min.band.idx", {
    ...
    expect_regression(giterator.intervals("1", ALLGENOME, iterator = itr), "giterator.cartesian_grid.min.band.idx")
