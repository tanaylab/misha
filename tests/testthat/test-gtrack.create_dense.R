...
test_that("test for dense track from intervals and values", {
    ...
    gtrack.create_dense(track_name, "Test dense track", intervals, values, 50, 0)

    # Extract data and check results
    r <- gextract(track_name, gintervals(c(1, 2), 0, 1000), colnames = "value")
...
test_that("test for overlapping intervals in dense track", {
    ...
    gtrack.create_dense(track_name, "Test overlapping intervals", intervals, values, 100, 0)

    # Extract data and check results
    r <- gextract(track_name, gintervals(1, 0, 400), colnames = "value")
...
test_that("test for values length not matching intervals", {
    ...
    expect_error(
        gtrack.create_dense(track_name, "Test mismatched lengths", intervals, values, 50, 0),
...
test_that("test for default value in dense track", {
    ...
    defval <- NaN
    gtrack.create_dense(track_name, "Test default value", intervals, values, 50, defval)
...
test_that("test for recreating test.fixedbin from extracted data", {
    ...
    gtrack.create_dense(track_name, "Recreated test.fixedbin", intervals, values, bin_size, 0)

    # Extract data from the recreated track
    recreated_data <- gextract(track_name, gintervals.all(), colnames = "value")

    # Compare the original and recreated data
    expect_equal(nrow(extracted_data), nrow(recreated_data))

    # Check track info
    new_info <- gtrack.info(track_name)
    expect_equal(new_info$type, "dense")
    expect_equal(new_info$bin.size, bin_size)

    expect_equal(gtrack.attr.get(track_name, "description"), "Recreated test.fixedbin")
    expect_equal(gtrack.attr.get(track_name, "binsize"), as.character(bin_size))
    expect_equal(gtrack.attr.get(track_name, "type"), "dense")
    expect_equal(gtrack.attr.get(track_name, "created.by"), paste0("gtrack.create_dense(", track_name, ", description, intervals, values, 50, 0)"))
})
