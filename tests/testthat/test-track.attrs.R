...
test_that("export track attributes without parameters", {
    ...
    # Filter out random tmptrack names from parallel tests (drop = FALSE to preserve data.frame)
    r <- r[!grepl("tmptrack_", rownames(r)), , drop = FALSE]
    # Remove test/polluting attributes if they exist (from previous/parallel tests)
    # These include test attributes and attributes added during track creation in other tests
    test_attrs <- c("testattr1", "testattr2", "binsize", "description", "type")
...
test_that("export specific attributes of tracks", {
    ...
    # Filter out random tmptrack names from parallel tests (drop = FALSE to preserve data.frame)
    r1 <- r1[!grepl("tmptrack_", rownames(r1)), , drop = FALSE]
    r2 <- r2[!grepl("tmptrack_", rownames(r2)), , drop = FALSE]

    # Remove test attributes if they exist (from previous/parallel tests)
    test_attrs <- c("testattr1", "testattr2")
    for (attr in test_attrs) {
        if (attr %in% names(r3)) {
            r3[[attr]] <- NULL
        }
...
test_that("export after importing modified attributes", {
    ...
    r1 <- gtrack.attr.export(attrs = c("testattr1", "created.by", "created.date", "created.user", "testattr2"))
    # Filter out random tmptrack names from parallel tests (drop = FALSE to preserve data.frame)
    r1 <- r1[!grepl("tmptrack_", rownames(r1)), , drop = FALSE]
    r1$testattr1 <- 1:dim(r1)[1]
    gtrack.attr.import(r1)
    r2 <- gtrack.attr.export(attrs = c("testattr1", "created.by", "created.date", "created.user", "testattr2"))
    r2 <- r2[!grepl("tmptrack_", rownames(r2)), , drop = FALSE]
...
test_that("import selected track attributes", {
    ...
    r1 <- r1[c("test.fixedbin", "test.sparse"), , drop = FALSE]
    gtrack.attr.import(r1)
    r2 <- gtrack.attr.export(attrs = c("testattr1", "created.by", "created.date", "created.user", "testattr2"))
    r2 <- r2[!grepl("tmptrack_", rownames(r2)), , drop = FALSE]
