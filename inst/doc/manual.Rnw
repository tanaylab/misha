...
Various functions in the library such as \texttt{gsample} make use of pseudo-random number generator. Each time the function is invoked the series of random numbers might differ. Hence two identical calls might produce different results. To guarantee reproducible results call \texttt{set.seed} before invoking the function.
